...
export async function POST(request: Request) {
  try {
    ...
    const result_from_deep_seek = await DEEP_SEEK_LLM.stream(messages_for_deep_seek);
    // old version without streaming
    ...
    return new Response(
      new ReadableStream({
        start : async (controller) => {
          for await (const chunk of result_from_deep_seek) {
